...
%         in_struc.min_count=110;  % Minimum count number below 1-peak mode.
%         in_struc.max_count=190;  % Maximum count number below 1-peak mode.
...
%             timebins_sec = (1e-15).*[-10000;-5000;-2000;-300;0;300;2000;5000;10000];
        elseif ismember(runnum,[225:257,267:325])
            in_struc.count_sum_low_limit=1.5e7;
            in_struc.count_sum_high_limit=2.5e7;
%             in_struc.count_sum_low_limit=1e7;
%             in_struc.count_sum_high_limit=3e7;
%             in_struc.count_sum_low_limit=2e7;
%             in_struc.count_sum_high_limit=3e7;
...
%             timebins_sec = (1e-15).*[-10000;-5000;-2500;-1000;-800;-600;-400;-200;0;200;400;600;800;1000;1500;2000;3000;5000;10000];
...
%     catch
%         disp(['Skipping step ',num2str(steploop),' for goodshotsonly error.']);
%         stepstoignore(steploop)=1;
%         continue
%     end
...
%     img_good_unbinned_tot = zeros([388 185 32]); img_noUV_unbinned_tot = zeros([388 185 32]);
    ...
    %% Generate new PixMap
%     [~,q,~,~,pixMap,phivalues,~,~,angleMap,phiMap] = image2rad(experiment,runnum,zeros([388,185,32]),D,ones([388,185,32]),Wavelength(s),x0,y0); % Generate new pixMap
...
%     [I_noUV_unbinned,~,~,~,~] = image2rad(experiment,runnum,photonimg_noUV_unbinned_avg,D,goodPixels,Wavelength(s),x0,y0);
    [I_noUV_unbinned,~,q,~,~,~] = image2rad(experiment,runnum,photonimg_noUV_unbinned_avg,zeros([388,185,32]),D,goodPixels,Wavelength(s),x0,y0); %bms, changed output
   
%     [I_noUV_unbinned_temp,q_temp,~,~,~,I_phi_noUV_unbinned,phivalues] = image2rad(experiment,runnum,photonimg_noUV_unbinned_avg,D,goodPixels,Wavelength(s),x0,y0);
    [I_noUV_unbinned_temp,~,q_temp,~,~,~,I_phi_noUV_unbinned,phivalues] = image2rad(experiment,runnum,photonimg_noUV_unbinned_avg,zeros([388,185,32]),D,goodPixels,Wavelength(s),x0,y0); %bms, changed output
